...
const PROJECT_LIST = [
  {
    ...
    title: '똑똑 365',
    description: [
      '홈 클리닝 애플리케이션. 정리수납과 가사 서비스를 원하는 시간에 예약하여 이용할 수 있는 서비스로 유저 전용 앱과 파트너 전용 앱으로 구성. 현재 애플 앱스토어와 구글 플레이스토어에 출시되어 상용중인 서비스.',
      '유저 앱 내의 서비스 신청 흐름을 구현하고, 백오피스 페이지의 유저 관리, 서비스 관리, 파일 업로드 및 다운로드 기능을 구현.',
      '메멘토 패턴을 사용해서 서비스 신청에 필요한 단계별 정보를 저장하도록 하여 유저가 신청을 중간에 중단하더라도 Zustand store 내에 저장이 되지 않도록 함.',
      'Next.js로 개발한 후 React Native 웹뷰를 사용하여 앱으로 출시하였고, 그 과정에서 브릿지 모듈을 사용하여 네이티브 기능을 사용할 수 있도록 함.',
      'zod를 사용하여 유효한 데이터를 검증할 수 있도록 하고, react hook form을 사용하여 폼을 관리하도록 함. 이를 통해 런타임에 타입의 안정성을 확보하고, 폼의 상태를 관리할 수 있도록 함.'
    ],
  ...
  },
  {
    ...
    description: [
      ...
      'jQuery로 이루어진 레거시 코드를 Angular2로 리팩토링.',
      '고객사 별로 다른 크기와 구조를 가진 로그 데이터들을 정형화된 데이터로 가공한 뒤, Google Chart API를 사용하여 일관된 분석 차트로 시각화.',
      '조건식을 통한 데이터 필터링 및 검색, 검색 결과 창 조인 기능, 엑셀 다운로드 기능을 구현.',
      '기존 온프레미스 환경에서 사용되던 로그 데이터 분석 시스템을 클라우드 환경으로 이전하는 작업을 수행.'
    ],
